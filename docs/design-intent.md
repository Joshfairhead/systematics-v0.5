...
| **Architectural Tetrad (the Model)** ⭐ | **Graph Template (++)** · **Topology (−−)** · **Geometry (+−)** · **Vocabulary (−+)** — the four base-space types. Graph Template = the source (order + size); the other three reconciled by it. **This is the Model node of the MVC triad**, and extends into the Architecture Pentad. | nodes=nouns (four base-space types) | code: `backend/src/core/{grammar,vocabularies}.rs`; `System` reconciles them | **Stage A rename DONE** in core: `Grammar`→`GraphTemplate`, `TopologicalVocabulary`→`Topology`, `GeometricVocabulary`→`Geometry` (46 tests pass; GraphQL `Gql*`/query names + serde field-names/ids kept stable → frontend/graph view unaffected; a coordinated wire + middleware rename is a follow-up). **Remaining Stage A:** explicit adjacency/incidence matrices, retire the `core/links.rs` shim (Holochain elements+links), expose order+degree as constraint-values. Links: *The architecture is an MVC triad*, [[architecture-pentad]] |
